...
void I_DPMI_Lock(void *data, unsigned int size)
{
    _dpmi_lockregion(data, size);
}
void I_DPMI_Unlock(void *data, unsigned int size)
{
    _dpmi_unlockregion(data, size);
}
...
void I_UpdateBox(int x, int y, int w, int h)
{
	...
	{
		outp(SC_INDEX+1,1<<i);
		dest = destscreen + y*SCREENWIDTH/4 + sp_x1*2;
		source = &screens[0][y*SCREENWIDTH+sp_x1*8+i];
		for (j = 0; j < h; j++)
		{
			count = sp_x2-sp_x1+1;
			if (count<0)
...
//
      ...
      //I_DPMI_Lock (&__begtext, &___argc - &__begtext);
